...
const ThemeToggle: React.FC = () => {
  ...
  if (!mounted) {
    return (
      <button
        type="button"
        className="absolute top-4 right-4 p-2 rounded-full bg-white/80 hover:bg-white/90 transition-colors duration-300 z-10"
  ...
  return (
    <button
      type="button"
      onClick={toggleTheme}
      className="absolute top-4 right-4 p-2 rounded-full bg-white/80 hover:bg-white/90 dark:bg-white/80 dark:hover:bg-white/90 transition-colors duration-300 z-10 bg-dark/90"
      aria-label="テーマを切り替える"
    >
      <img
        src="/data/sun.svg"
